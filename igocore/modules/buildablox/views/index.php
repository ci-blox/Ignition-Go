	...
	<table class="table table-striped">
		<thead>
			<tr>
				...
				<th><?php echo lang('mb_table_dbversion_current_available'); ?></th>
				<th colspan="2"><?php echo lang('mb_actions'); ?></th>
			</tr>
		</thead>
		<tbody>
		<?php 
		foreach ($modules as $module => $config) : ?>
			<tr>
				...
				<input type="submit" class="btn btn-success btn-xs" onclick="return confirm('<?php e(js_escape(lang('mb_migrate_confirm'))); ?>');" value="<?php e(lang('mb_form_actions_migrate_to').' '.str_pad(number_format($dbv['installed'],0)+1, 3, '0', STR_PAD_LEFT)) ?>" />
				<?php }
				else if ($dbv['installed']==$dbv['latest']) { ?>
				<input type="submit" class="btn btn-danger btn-xs" onclick="return confirm('<?php e(js_escape(lang('mb_migrate_rollback_confirm'))); ?>');" value="<?php e(lang('mb_form_actions_rollback_to').' '.str_pad(number_format($dbv['installed'],0)-1, 3, '0', STR_PAD_LEFT)) ?>" />
				<?php }
				  echo form_close(); 
				} ?>
				</td>
